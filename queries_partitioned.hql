...
FROM 
    crime_partitioned c
JOIN 
    segment_partitioned s ON c.segment_id = s.id
JOIN 
    vertice_partitioned v_start ON s.start_vertice_id = v_start.id
JOIN 
    vertice_partitioned v_end ON s.final_vertice_id = v_end.id
JOIN 
    district_partitioned d_start ON v_start.district_id = d_start.id
JOIN 
    district_partitioned d_end ON v_end.district_id = d_end.id
JOIN 
    time_partitioned t ON c.time_id = t.id
WHERE 
...
JOIN 
    vertice_partitioned v_end ON s.final_vertice_id = v_end.id
JOIN 
    neighborhood_partitioned n ON (v_start.neighborhood_id = n.id OR v_end.neighborhood_id = n.id)
JOIN 
    time_partitioned t ON c.time_id = t.id
WHERE 
